...
num = 0
display = ''
number = Label(root, text=display, font=('TkDefaultFont',25))

anslabel = Label(root, text='Answer:')
ansbox = Entry(root, width=10)
ansbutton = ttk.Button(root, text='Check', command= lambda: check())
...
def letsgo():    
    ...
    try:
       minval = int(minimum.get())
    except:
        minval = 0
    
    try:
        maxval = int(maximum.get())
    except:
        if mult == 1 or div == 1:
            maxval = 10
        else:
    ...
    if guess == answer:
        checklabel = Label(root, text='Correct')
    else:
        checklabel = Label(root, text='Incorrect')
